...
Scientific rationale (Nature Geoscience review compliance):
  1. The AEBP v3.1 forward model uses CONTINUOUS functions (no arbitrary
     thresholds). Adding Al_norm > 0.5 "activation threshold" introduces
     three non-physical parameters not present in the original model.
  2. The GP Emulator (R2 = 0.766 ± 0.021, 5-fold CV) captures the full
     AEBP response surface without ad hoc modifications.
  3. The detection threshold (T > 0.4) was validated against LPIA
     observations (T_obs = 0.52). Raising to 0.7 is post-hoc.
  4. Sim 4 (volcanic, T = 0.452) represents genuine SCIENTIFIC BEHAVIOUR:
     a 4/5-condition scenario can produce marginal signal. This is a
     discovery, not a bug.
...
notes = {
    ...
    4: "Dust flux 100x — Al_norm=0.1, weak but non-zero AEBP effects (MARGINAL SIGNAL)",
